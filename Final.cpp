...
/*
 ...
 抓資料速度快
 只要給索引就可以抓到想要的資料
 當data size 較小時，使用array 會比較清楚和易操作資料
 且array
 可以檢索特定標號(index)的元素。
 可以按照標號增加的順序遍歷串列。
 ...
 第11題:
 宣告兩個pointer, say x & y，然後分別紀錄第x,y node 的 linked list 的值，分別是H[x],H[y]，其中x為第2i的node，y為第i的node，where i = 正整數，if we can find H[x] = H[y]，then we can say that there is a cycle in the linked list。
